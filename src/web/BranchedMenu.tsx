import { isValidElement, ReactNode, useLayoutEffect, useRef, useState } from 'react';
...
const renderIcon = (icon: unknown) =>
  isValidElement(icon) ? icon : icon ? <HugeiconsIcon icon={icon as Parameters<typeof HugeiconsIcon>[0]['icon']} size={16} strokeWidth={1.8} /> : null;
  ...
  return (
    ...
    >
      ...
      {items.map((item, i) => {
        ...
        const leafValue = item.value ?? String(item.label);
        const leafActive = !kids && leafValue === active;
        const bodyH = kids ? PAD * 2 + kids.length * rowHeight : 0;
        const key = item.value ?? String(item.label) ?? `section-${i}`;
        return (
