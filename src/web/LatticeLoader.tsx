...
const resolvePattern = (pattern: string | CustomPattern, grid: number): PatternDefinition => {
  ...
  const numericCells = cells.filter((v): v is number => v != null);
  const max = Math.max(0, ...numericCells);
...
export default function LatticeLoader({
  ...
  shape = 'round',
  color = 'currentColor',
  doneColor = 'var(--green, #22c55e)',
  errorColor = 'var(--danger, #ef4444)',
  cellSize = 6,
  gap = 2,
  fontSize = 14,
  step = 90,
  ...
  const mark: 'done' | 'error' = status === 'working' ? markRef.current : status;
